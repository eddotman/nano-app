...
!     work from user fortran program.
! 
!     Some are from mpich's mpif.h, others are custom.
!
!     Author: Brian P. Walenz
!             modified by F.Sever (derived from fakempi_h)
!     Modifications done:
!     - added among the list of functions the function MPI_Reduce.
!     - commented lines with function list since not needed for 
!       user program in fortran

      integer MPI_BYTE, MPI_CHARACTER, MPI_COMPLEX
      integer MPI_DOUBLE_PRECISION, MPI_INTEGER, MPI_REAL, MPI_REAL8
...
!     external MPI_Type_struct, MPI_Reduce
