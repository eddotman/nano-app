...
! neanmoins pop etaient modifiees de ce que l'atome soit neutre      
      ...
      call dirac(h_ray,icheck,ibav,irel,lqn,n_orb,n_ray,nnlm,nqn,
     &                 pop,psi,ray,ray_max,rho,rqn,Z)
...
! Recuperation de la fonction d'onde de l'orbitale de valence potentiellement occupee
        do io = n_orb,1,-1
          ...
          if( irel == 1 .and. lqn(io) /= 0 ) then
            psi_open_val(1:n_ray,1) = 0.5_db 
     &                         * ( psi(1:n_ray,io) + psi(1:n_ray,io-1) )
...
! Les fonctions d'onde psi sont multipliees par r
      ...
      do io = 1,n_coeur
        rho_coeur(1:n_ray,it) = rho_coeur(1:n_ray,it)
     &                        + pop(io) * psi(1:n_ray,io)**2 
      end do

      rho_coeur(1:n_ray,it) = rho_coeur(1:n_ray,it)
     &                      / ( quatre_pi * ray(1:n_ray)**2 )
      ...
      select case (Z)
        ...
        case default   
           do i = 1,2
             ...
             ncoeur(i,it) = nqn(io)
             if( irel == 1 .and. lqn(io) /=0 ) then
               psi_coeur(1:n_ray,i,it) = 0.5_db * ( psi(1:n_ray,io)
     &                                            + psi(1:n_ray,io-1) ) 
             else 
...
! on prend en compte la charge eventuelle de l'atome
  ...
  120 format(///'   n_ray =',i5,' > nrm =',i4,/
     &  ' Use keyword n_ray in the indata file to decrease its value !')
  130 format(///'   n_orb > nnlm ',//
     &          ' Increase nnlm in lecture.f !')
  135 format(/'  it =',i3,//
     &   '      rato    psi_coeur(n=',i1,',l=',i1,')  ',
     &   'psi_val(n=',i1,',l=',i1,')')
...
! This was corrected by Bengt Lindgren and A. Rosen Dec 1987.
      ...
      subroutine dirac(h_ray,icheck,ibav,irel,lqn,n_orb,n_ray,nnlm,nqn,
     &                 pop,psi,ray,ray_max,rho,rqn,Z)
      ...
      real(kind=db), dimension(n_ray):: a, b, ray, rh, rho, rj, vr, Vrf,
     &                                  vs, y 

      common/iblock/ nc1, npts, jspn
      common/rblock/ rn, h, zeff, xion, phi, eps, del,
     & xalph, xlatt, rnuc, fj, v0,
     & da(5), dbb(5), a0(15), b0(15), voc(5), anuc, sumel,
     & rbar, rba2, vbar, h3, fk, cs, g, q11,
     & q22, tcs
...
! IDIRC= ZERO FOR HARTREE-FOCK-SLATER, NONZERO FOR DIRAC-SLATER
      ...
      call dinpt(Delta_rv,h_ray,ibav,icheck,idirc,lqn,n_orb,n_ray,nnlm,
     &                 nqn,ph,pop,ray,ray_max,rh,rj,rqn,vr,vs,
     &                 xe,xj,xl,xn,xz,Zn)
...
! RH=ELECTR.DENS. RJ=SPIN.DENS. VR=R*POT.
        call dipot(a,b,Convr,Delta_rv,E_v,E_Vxc,E_xc,E_Z,ibav,icheck,
     &             idirc,1,n_orb,n_ray,Q,ray,rh,rj,vr,vs,xte,y,Zn)
        ...
        Summa = 0._db
        ps = 1 - ph
        if( icheck > 2 .or. (icheck > 1 .and. last ) )
     &                                    write(ibav,20) ncycl, ph
        ...
        do i = 1, n_orb
          ...
          if( i == jspn + 1 ) then
            call dipot(a,b,Convr,Delta_rv,E_v,E_Vxc,E_xc,E_Z,ibav,
     &                 icheck,idirc,2,n_orb,n_ray,Q,ray,rh,rj,vr,vs,
     &                 xte,y,Zn)
...
! SKIP UNOCCUPIED STATES UNTIL LAST CYCLE.
          ...
          if( idirc == 0 ) then
            call hsdif(a,b,e,ibav,icheck,last,lq,n_ray,nq,ray,vr,y,Zn)
            y(1:n_ray) = ph * xz(i) * a(1:n_ray)**2
          else
            call didif(a,b,e,ibav,icheck,last,lq,n_ray,nq,ray,vr,y,Zn)
        ...
        if( icheck > 2 ) write(ibav,23) E_Vxc, E_v, E_Z, Summa, 
     &                                  rh(1), E_total
      ...
      call diout(E_total,E_v,E_Vxc,E_xc,E_Z,ibav,icheck,
     &           idirc,n_orb,n_ray,nnlm,psi,psi_small,Q,ray,rh,rho,
...
! n_orb : NUMBER OF SHELLS
      ...
      subroutine dinpt(Delta_rv,h_ray,ibav,icheck,idirc,lqn,n_orb,n_ray,
     &                 nnlm,nqn,ph,pop,ray,ray_max,rh,rj,rqn,vr,vs,
     &                 xe,xj,xl,xn,xz,Zn)
     ...
     & q22, tcs
      ...
      parameter( adz = 0.3_db, aez = .25e4_db, afz = .1e-6_db,  
     &           agz = .1e-4_db, arz = .208e-4_db)
...
! XZ = NUMBER OF ELECTRONS IN SHELL
        ...
        if( abs(xe(i)) < afz ) xe(i) = - 0.5_db * (zbar / xn(i))**2
     &                               + vbar
        if( idirc == 0 ) xj(i) = 0._db
        if( icheck > 2 ) write(ibav,210) xn(i), xl(i), xj(i), xe(i),
     &                                  xz(i)
        if( xn(i) < 1-eps6 ) exit
        sum = sum + xz(i)
        if( xn(i) == xl(i) ) exit
...
! SET STARTING SPIN DENSITY = .5*CHARGE DENS.
      if( rj(1) == 0._db ) rj(1:n_ray) = 0.5_db * rh(1:n_ray)
  ...
  150 format(/' PARAMTERS TO GENRT ADDTNL BASIS FNS ',
     &' RADIUS OF POTL WELL',f12.8,' ITS DEPTH',f12.8/,' CUTOFF',f12.8)
...
! POINT ION ONLY OPTION FOR RHO(1).LT.0
      ...
      subroutine dipot(a,b,Convr,Delta_rv,E_v,E_Vxc,E_xc,E_Z,ibav,
     &                 icheck,idirc,isw,n_orb,n_ray,Q,ray,rh,rj,vr,vs,
     &                 xte,y,Zn)
     ...
     & q22, tcs
      ...
      real(kind=db), dimension(n_ray):: a, b, c, ray, rh, rhd, rj, vr,
     &                                  vs, y, y2

      equivalence (a0(1), dc(1))
...
!...CUBIC INTERPOLATION
        ...
        E_Z = E_Z - Zn * ( ( ( 1.5_db * anc / rnuc) - bnc )
     &               - 0.5_db * cnc / rnuc**3 )
      endif
...
!***********************************************************************
      ...
      xfx = x * x + bf * x + cf
      s4 = s**4 - 1
      fs = ((((1 + s)**fothi) + ((1 - s)**fothi)) - 2)
     &   / (2._db**fothi - 2)
      beta = 1._db / ( 2.74208 + 3.182*x + 0.09873*x**2 + 0.18268*x**3)
      dfs = (fothi * (((1 + s)**third) - ((1 - s)**third)))
     &    / (2._db**fothi - 2)
     ...
     &    - xf0)**2) / xfx) + cf2 * atnf)))
      ...
      tp1 = (x * x + bp * x) / xpx
      tf1 = (x * x + bf * x) / xfx
      ucp = ecp - ((ap / 3) * (1._db - tp1 - (cp3 * ((x / (x - xp0) - 
     &      tp1) - ((xp0 * x) / xpx)))))
      ucf = ecf - ((af / 3) * (1._db - tf1 - (cf3 * ((x / (x - xf0) - 
     &      tf1) - xf0 * x / xfx))))
      uc0 = ucp + (ucf - ucp) * fs
      uc10 = uc0 - (ecf - ecp) * (s - 1) * dfs
      uc20 = uc0 - (ecf - ecp) * (s + 1) * dfs
      duc = (ucf - ucp) * beta * s4 * fs
     &    + (((((ecf - ecp) * (- rs / 3)) * dbeta) * s4) * fs)
      duc1 = duc - (ecf - ecp) * beta * (s - 1)
     &           * (4*(s**3)*fs + s4 * dfs)
      duc2 = duc - (ecf - ecp) * beta * (s + 1)
     &           * (4*(s**3)*fs + s4 * dfs)
      uc1 = uc10 + duc1
      uc2 = uc20 + duc2
...
!     MODIFIED FOR CONTINUUM STATES 1978....GREG BENESH...
      ...
      subroutine hsdif(a,b,e,ibav,icheck,last,lq,n_ray,nq,ray,vr,y,Zn)
     ...
     &   abb = 1.02_db, fmodc = .92561983_db, fcorc = .07438017_db,  
     &   epx = 625._db, smset = 1.e20_db)
...
! vr = r * V(r)
        ...
        do k = 5, ki
          r = ray(k)
          bpred = da(1) + ha1 * ( dbb(4) - 0.5_db * dbb(3) + dbb(2) )
          apred = a(k-4) + ha1 * ( da(4) - 0.5_db * da(3) + da(2) )
          bmode = bpred + fmodc * bdif
          amode = apred + fmodc * adif
          fmode = - lqpo * bmode + b(k) * amode
          bcorr = aiz * da(4) - ajz * da(2)
     &           + ha2 * ( fmode + 2 *  dbb(4) - dbb(3) )
          acorr = aiz * a(k-1) - ajz * a(k-3) 
     &          + ha2 * ( bmode + 2 * da(4) - da(3) )
          adif = acorr - apred
          bdif = bcorr - bpred
...
!===>CONTINUUM STATE BY WKB ,FITTED TO OUTWARD INTEGRATION...
        if( Continuum ) then
          ...
          b(is) = 0._db
          b(is + 1) = (h / 24) * (((9 * chi(is) + 19 * chi(is + 1))
     &              - (5 * chi(is + 2))) + chi(is + 3))

          in = is + 2
          do i = in, n_ray
            b(i) = ( h / 3 ) * ( chi(i-2) + 4*chi(i-1) + chi(i) )
     &           + b(i-2)
          end do
          ...
          do i = is, nm4
            ...
            rg = rst * dg**i
            do k = i, i4
              chix(k+1-i) = (facts / chi(k)) * cos( b(k) - beth )
     &                    / ray(k)
...
!  CORR OCT79,NOTE R**(L+1) SCALING ON A(K) AND DP/DT, WHERE T=LN(R)...
  ...
  515   continue   ! Point d'arrivee en cas de Continuum
...
!===>TEXT FOR ENERGY CONVERGENCE....
      ...
      if( icheck > 2 .and. niter > 10 )
     &                         write(ibav,13) nq, lq, emin, e, emax
...
!  FINDS EIGENVALUES. NORMALIZES ORBITAL FUNCTIONS.
      ...
      subroutine didif(a,b,e,ibav,icheck,last,lq,n_ray,nq,ray,vr,y,Zn)

      use declarations
      implicit real(kind=db) (a-h,o-z)
      
      parameter( ga1 = 251._db, ga2 = 646._db, ga3 = 264._db,
     & ga4 = 106._db, ga5 = 19._db, ga6 = 116._db, ga7 = 496._db,
     & ga8 = 96._db, ga9 = 16._db, ga10 = 4._db, ga11 = 81._db,
     & ga12 = 306._db, ga13 = 216._db, ga14 = 126._db, ga15 = 9._db,
     & ga16 = 56._db, ga17 = 256._db, ga18 = 96._db, ga19 = 256._db,
     & ga20 = 56._db )
...
!   PREPARE CONSTANTS USED FOR THIS ORBITAL
    ...
    1 call didf1(a,b,e,ki,lq,n_ray,nb_nodes,ray,vr)

!  nb_nodes = NUMBER OF NODES + L+ 1. nb_nodes SHOULD EQUAL nq.
      if( icheck > 3 ) write(ibav,10) nq, lq, fj, emin, e, emax

!  TOO MANY NODES.
      if (nq - nb_nodes) 2, 4, 13
    2 if (e .lt. emax) emax = ( 2 * emax + e) / 3
      dele = max(e,emin - e)
      e = e + (0.5_db * dele)
      dl1 = abs(emax - emin) / (abs(e) + 1)
      if (dl1 - delk) 3, 1, 1
    3 if( icheck > 2 ) write(ibav,15) nb_nodes, nq, lq, fj, emin, e,emax
      emin = aaz * emin

      goto 1

!  CORRECT NUMBRR OF NODES
    4 ra = a(ki)
      rb = b(ki)
      do i = 1, ki
        a(i) = a(i) / ra
  ...
  402 rz = - vr(kj) / r
     ...
     &                    + ga19 * dbb(4) + ga20 * dbb(5)) * ha9)
      ...
      do k = kj-4,ki,-1
        rp21 = ( e * ray(k) - vr(k) ) / cs
        rp12 = - rp21 - tcs * ray(k)
        akk = a(k+4) - ha1 * (da(3) - 0.5_db * da(2) + da(1))
        bkk = b(k+4) - ha1 * (dbb(3) - 0.5_db * dbb(2) + dbb(1))
        da(4) = q11 * akk + rp12 * bkk
        dbb(4) = q22 * bkk + rp21 * akk
        a(k) = a(k+1) - ha2 * da(4) - ha3 * da(3) + ha4 * da(2)
     &       - ha5 * da(1)
        b(k) = b(k+1) - ha2 * dbb(4) - ha3 * dbb(3) + ha4 * dbb(2)
     &       - ha5 * dbb(1)
        da(1) = da(2)
        dbb(1) = dbb(2)
        da(2) = da(3)
...
c===>REDUCE DE AND TRY AGAIN......
   12 if( icheck > 2 ) write(ibav,40) nq, lq, fj, emin, e, emax, de
      de = 0.5_db * de
...
c  TOO FEW NODES
   ...
   13 if (e > emin) emin = e
      ...
      dl1 = abs(emax - emin) / (abs(e) + 1)
      if (dl1 - delk) 14, 1, 1
   14 if( icheck > 2 ) write(ibav,50) nb_nodes, nq, lq, fj, emin, e,emax
      if( ntest /= 0 ) then
        call write_error
        do ipr = 3,9,3
          write(ipr,60)
 ...
 1920 call write_error
   ...
   19 format(//' Dirac routine:'/,
     &       ' THE TURNING POINT IS BEYOND RN. A LARGER VALUE OF RN',
     &       ' MAY BE NEEDED.')
...
!  MAJOR COMPONENT OF THE RADIAL WAVE FUNCTION.
      ...
      subroutine didf1(a,b,e,ki,lq,n_ray,nb_nodes,ray,vr)
...
!   DIOUT ALSO COMPUTES AND PRINTS (1/R) AND (1/R**3)- INTEGRALS
      ...
      subroutine diout(E_total,E_v,E_Vxc,E_xc,E_Z,ibav,icheck,
     &                 idirc,n_orb,n_ray,nnlm,psi,psi_small,Q,ray,rh,
...
! E_Z = Integrale( (Z/r)*rho*d3r )
      ...
      if( icheck > 1 ) then
        ...
        do i = 1, n_orb
          ni = nist(i)
          write(ibav,14) nlj(ni), xn(i), xl(i), xj(i),
     &                   xz(i), Hartree * xe(i)
        end do
        write(ibav,15) Z, Q, Hartree * E_total, Hartree * (E_Z + U),
     &                 Hartree * E_xc, Hartree * Ekin, 
     &                 Hartree * Summa, Hartree * U,     
     &                 Hartree * E_Vxc
      endif
      ...
      if( icheck > 1 ) then
        ...
        do i = 1,n_ray
          f = quatre_pi * ray(i)**2
          write(ibav,120) bohr*ray(i), Hartree*Vrf(i)/ray(i), f*rho(i),
     &                    psi(i,1:n_orb) 
        end do
        if( idirc/= 0 ) then
          ...
          do i = 1,n_ray
            f = quatre_pi * ray(i)**2
            write(ibav,120) bohr*ray(i), Hartree*vrf(i)/ray(i),
     &                      f*rho(i), psi_small(i,1:n_orb) 
          end do
        endif
      endif
   ...
   15 format(/' Nuclear charge                  =',i12/,
     &        ' Integral of charge density      =',f12.3/,
     &        ' Total energy = Ue + U + Exc + T =',f12.3/,
     &        ' Potential energy = U + Ue       =',f12.3/,
     &        ' Exchange energy = Exc           =',f12.3/,
     &        ' Kinetic energy = T              =',f12.3/,
     &        ' Sum of the energy eigenvalues   =',f12.3/,
     &        ' 0.5*Integral(Vh.rho.dr)         =',f12.3/,
     &        ' Integral(Vxc.rho.dr)            =',f12.3)
  110 format(/'      Radius    Potential  4*pi*r2*Rho ',36(4x,a6,3x))
  120 format(f13.6,1p,38e13.5)
  130 format(/'  Small component:',//
     &        '      Radius    Potential  4*pi*r2*Rho ',36(4x,a6,3x))
      end

!***********************************************************************

! BLOCKDATA LABELS FOR DIOUT
