...
! Calculation of the minimum of metrics in a grid of points.
      ...
      subroutine minim(fdmfit_out,index_Met_Fit,Minim_fdm_ok,minimok,
     &             ncali,ndm,ngroup_par,ngroup_par_conv,nmetric,
     &             nmetricm,Nom_Met,nparam,npm,par,par_op,typepar)
      ...
      real(kind=db), dimension(ngroup_par) :: a, b, parm_ext, par_op,
     &                                       parp_ext, sol, sm
      real(kind=db), dimension(ngroup_par,nmetric) :: distm, par_pol,
     &                                               par_ell
...
! Cas de la courbe convexe.
            ...
            distm(igr,im) = a(igr) * par_pol(igr,im)**2
     &                    + b(igr) * par_pol(igr,im) + dmin
            if( distm(igr,im) < 0.5 * dmin ) then
              distm(igr,im) = dmin
              par_pol(igr,im) = par(igr,ia(igr,im))
            else  
              if( igr > 1 .and. im == 1 ) polynome = .true.
              if( igr > ngroup_par_conv .and. im == 1 )
     &                                    polynomefdm = .true.
...
! Calcul de l'hyperellipsoide.
        ...
        do igr = 1,ngroup_par
          ...
          do jgr = 1,igr - 1
            ...
            ifac = 0
            do i = -1,1,2
              if( ( ia(igr,im) == 1 .and. i == - 1 )
     &           .or. ( ia(igr,im) == nparam(igr) .and. i == 1 ) ) cycle
              do j = -1,1,2
                if( ( ia(jgr,im) == 1 .and. j == - 1 )
     &           .or. ( ia(jgr,im) == nparam(jgr) .and. j == 1 ) ) cycle
                ifac = ifac + 1
                ic(:) = ia(:,im)
                ic(igr) = ia(igr,im) + i
                ic(jgr) = ia(jgr,im) + j
                ...
                ab(igr,jgr) = ab(igr,jgr)
     &                      + ( dd - a(igr)*xi**2 - a(jgr)*xj**2
     &                        - b(igr)*xi - b(jgr)*xj ) / ( xi * xj ) 
              end do
            end do
...
! Metrique minimum
        ...
        do igr = 1,ngroup_par
          ...
          i = i + 1
          j = 0
          distmin_ell(im) = distmin_ell(im)
     &                    + a(igr) * sol(i)**2 + b(igr)*sol(i)
          do jgr = 1,ngroup_par
            if( nparam(jgr) < 3 .or. igr == jgr ) cycle
            j = j + 1
            distmin_ell(im) = distmin_ell(im)
     &                      + ab(igr,jgr) * sol(i) * sol(j)
          end do
        end do
        ...
        if( distmin_ell(im) > dmin + 0.00000001_db .or.
     &      distmin_ell(im) < 0.5 * dmin ) then
          if( im == 1 ) ellipse = .false.
          distmin_ell(im) = dmin
          do igr = 1,ngroup_par
            par_ell(igr,im) = par(igr,ia(igr,im))
...
! Ecriture des coupes autour des minima de la grille.
      ...
      if( ncali > 1 ) then
        do igr = 1,ngroup_par
          if( nparam(igr) < 2 ) cycle
          write(4,140) adjustl(typepar(igr)), adjustr(typepar(igr)),
     &                 Nom_Met(1:nmetric)
          do ip = 1,nparam(igr)
            write(4,150) par(igr,ip),
     &                   ( dist_cut(igr,ip,im), im = 1,nmetric )
          end do
          if( nmetric == 3 ) then
          ...
          if( ngroup_par > 1 ) 
     &          write(4,'(A)') '      with the other parameters :' 
          do jgr = 1,ngroup_par
            if( jgr == igr ) cycle
      ...
      return
  110 format(//'  Minimum for the second order polynomia :',
     &                         /18x,4(3x,a2,'min',3x))
  115 format(16x,2f11.5,2f11.6)
  120 format(5x,a9,' =',4f11.5)
  130 format(/'  Minimum for the hyperellipsoid :',
     &                         /18x,4(3x,a2,'min',3x))
