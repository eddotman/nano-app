...
!***********************************************************************
      ...
      subroutine atom(clementi,com,icheck,icom,itype,jseuil,lcoeur,
     &      lseuil,lvval,mpinodes,mpirank,nbseuil,ncoeur,ngroup,nlat,
...
! Valeurs pour l'atome excite non encore utilises
            ...
            call dirgen(icheck,it,0,jseuil,lcoeur,lqnexc,
     &        lseuil,lvval,mpirank,n_orbexc,nbseuil,ncoeur,nlat,nlatm,
     &        nnlm,nonexc,nqnexc,nr,nrato_dirac,
     &        nrm,nseuil,nspin,ntype,nvval,pop_open_val,popatc,popatv,
     &        popexc,popval,psi_coeur,psii,psi_open_val,psival,
     &        rr,rho_coeur,rhoit,numat(it),relativiste)
          ...
          case(2)
            ...
            call clem(icheck,it,0,lseuil,lvval,mpirank,nbseuil,nlat,
     &             nlatm,nonexc,nr,nrm,nseuil,ntype,nvval,popatc,popatv,
     &             psii,psival,rr,rhoit,numat(it))
            nrato(it) = nr
            rato(1:nr,it) = rr(1:nr)
            rato(0,it) = 0._db
...
! on prolonge
     ...
     &                                           MPI_COMM_WORLD,mpierr) 
              ...
              if( it == 0 ) call MPI_Bcast(psii,nbseuil*nrm,MPI_REAL8,0,
     &                                           MPI_COMM_WORLD,mpierr)  
            endif
      ...
      return
  110 format(/' nrato =',i6,' > nrm =',i6,' !',
     &       /' Increase the value of nrm using keyword nrato')
  120 format(/' The number of radius needs to be increased',/
     &        ' nrato =',i6,' > nrm =',i6,' after extrapolation !',
     &       /' Increase the value of nrm using keyword nrato')
...
!
      subroutine clem(icheck,it,itabs,lseuil,lvval,mpirank,nbseuil,nlat,
     &            nlatm,nonexc,nrato,nrm,nseuil,ntype,nvval,popatc,
...
! excite
     ...
     &                    * rato(ir)**nexp(io,j) * exp( - arg )
            ...
            if( icheck > 1 ) write(3,120) io, nprin(io), nazim(io),
     &                                     nconf(io), ch
          endif
        endif
...
!  (see CR p.180, equation (6))
  ...
  105 format(/' Number of radius =',i6,' > nrm =',i6,//
     &        ' Increase nrm using keyword nrato !'/)
...
! symetrie relative.
      ...
      subroutine symsite(absauto,angxyz,Atom_with_axe,Atom_nonsph,
     &      Atom_nsph_e,Axe_atom_gr,axyz,Base_ortho,Cubmat,dcosxyz,
     &      Flapw,iabsm,icheck,igr_i,igr_is,igr_proto,itype,
     ...
     &      popats,posn,Struct)
      ...
      logical absauto, Atom_nonsph, base_ortho, flapw, 
     &        Magnetic, matper, Memory_save, mspinor
      logical, dimension(ngroup):: Atom_nsph_e, ok
      logical, dimension(0:ngroup_m):: Atom_with_axe
      logical, dimension(:), allocatable:: Far_atom

      real(kind=db), dimension(3):: angxyz, Axe_atom_c, axyz, dcosxyz,
     &            ps, spini, vspin, vspini, wspin, wspini
      real(kind=db), dimension(3,3):: Cubmat, Cubmati, matopsym 
      real(kind=db), dimension(3,ngroup):: Axe_atom_s,
     &                                    pos, posg, posn, poss
...
! Recherche de la  symetrie qui rend les atomes equivalents.
     ...
     &                                                   cycle boucle_is
                  ...
                  if( .not. ( Magnetic .or. Atom_nonsph ) )
     &                                                   cycle boucle_ia

                  if( .not. ( Atom_with_axe(ia) .or. Atom_with_axe(ib)))
     &                                                   cycle boucle_ia

                  if( ( Atom_with_axe(ia) .and. .not. Atom_with_axe(ib))
     &                 .or.
     &              ( .not. Atom_with_axe(ia) .and. Atom_with_axe(ib) ))
     &                                                   cycle boucle_is
...
! On prend les axes de spins en reference au premier atome du groupe
        if( Magnetic .or. Atom_nonsph ) then
          ...
          do i = 2,neq
            ...
            Axe_atom_c(:) = Axe_atom_gr(:,igreq(1))
            if( abs(Axe_atom_c(1)) < eps6 ) then
              wspin(1) = 1._db;  wspin(2:3) = 0._db
            else
              wspin(3) = 1._db;  wspin(1:2) = 0._db
...
!*********************************************************************
      ...
      subroutine pop_proto(chargat,Charge_free,chargm,flapw,iabsm,
     &      icheck,igreq,itype,mpirank,n_atom_proto,n_multi_run,neqm,
     ...
     &      popatm,popats,run_done)
      ...
      call pop_group(chargatg,Charge_free,chargm,flapw,icheck,itype,
     &      mpirank,ngroup,nlat,nlatm,nspin,ntype,numat,popatc,popats)
...
! Sousprogramme elaborant la grille en energie pour le calcul de XANES
     ...
     &      icheck,lin_gam,ngamme,neimagent,nenerg)
...
!***********************************************************************
      ...
      subroutine init_run(com,ecrantage,
     &    Force_ecr,Hubb,iabsorbeur,iabsorig,icheck,icom,
     &    itabs,itype,jseuil,lcoeur,lecrantage,lqnexc,lseuil,lvval,
     &    mpinodes,mpirank,n_multi_run,n_orbexc,nbseuil,ncoeur,
     ...
     &    relativiste,rho_coeur,rhoit,rmt,rmtimp,V_hubbard)
      ...
      logical:: force_ecr, nonexc, relativiste
      logical, dimension(0:ntype):: hubb

      real(kind=db), dimension(0:ntype):: popatc, rchimp, rmt, rmtimp,
     &                                     V_hubbard
      ...
      call type_work(com,ecrantage,force_ecr,hubb,
     &      iabsorbeur,icheck,icom,itabs,itype,jseuil,lcoeur,lecrantage,
     &      lqnexc,
     &      lseuil,lvval,mpinodes,mpirank,n_orbexc,nbseuil,ncoeur,
     &      necrantage,ngroup,nlat,nlatm,nnlm,nompsii,nonexc,nqnexc,
     &      nrato,nrato_dirac,nrm,nseuil,nspin,ntype,
     &      numat,nvval,pop_open_val,popexc,popatc,popats,popatv,popval,
     &      psi_coeur,psii,psi_open_val,psival,rato,rchimp,
     &      relativiste,rho_coeur,rhoit,rmt,rmtimp,V_hubbard)
...
!*********************************************************************
      ...
      subroutine type_work(com,ecrantage,force_ecr,hubb,
     &      iabsorbeur,icheck,icom,itabs,itype,jseuil,lcoeur,lecrantage,
     &      lqnexc,
     &      lseuil,lvval,mpinodes,mpirank,n_orbexc,nbseuil,ncoeur,
     &      necrantage,ngroup,nlat,nlatm,nnlm,nompsii,nonexc,nqnexc,
     ...
     &      relativiste,rho_coeur,rhoit,rmt,rmtimp,V_hubbard) 
      ...
      real(kind=db), dimension(0:ntype) :: popatc, rchimp, rmt,
     &                                       rmtimp,V_hubbard
      real(kind=db), dimension(nspin) :: ecrantage
      real(kind=db), dimension(nrm):: rr
      ...
      call screendef(ecrantage,force_ecr,icheck,itype(iabsorbeur),
     &      lecrantage,lqnexc,lseuil,lvval,mpirank,n_orbexc,necrantage,
     &      nlat,nlatm,nnlm,nqnexc,
     &      nseuil,nspin,ntype,nvval,popexc,popval,numat(itabs))
...
! Il faut recalculer cette partie meme en nonexc pour avoir psii.
      ...
      if( icom(itabs) == 2 .or. nompsii == 'clementi' ) then
        ...
        call clem(icheck,itabs,itabs,lseuil,lvval,mpirank,nbseuil,nlat,
     &        nlatm,nonexc,nr,nrm,nseuil,ntype,nvval,popatc,popatv,
     &        psii,psival,rr,rhoit,numat(itabs))

      elseif( icom(itabs) == 1 .or. nompsii == 'dirac' ) then 

        call dirgen(icheck,itabs,itabs,jseuil,lcoeur,lqnexc,
     &       lseuil,lvval,mpirank,n_orbexc,nbseuil,ncoeur,nlat,nlatm,
     &       nnlm,nonexc,nqnexc,nr,nrato_dirac,
     &       nrm,nseuil,nspin,ntype,nvval,pop_open_val,popatc,popatv,
     &       popexc,popval,psi_coeur,psii,psi_open_val,psival,
     &       rr,rho_coeur,rhoit,numat(itabs),relativiste)
...
! popats pour l'absorbeur        
     ...
     &              .and. nompsii /= 'dirac' ) then
        ...
        if( mpinodes > 1 ) then
          ...
          call MPI_Bcast(psii,nrm*nbseuil,MPI_REAL8,0,MPI_COMM_WORLD,
     &                   mpierr)
          call MPI_BARRIER(MPI_COMM_WORLD,mpierr)
        endif 
      endif   
...
! rayons FLAPW
        ...
        do ir = 1,nrato(itabs_t)
          do irt = 2,nr
            if( rr(irt) > rato(ir,itabs_t) ) goto 1040
          end do
          exit
 1040     p1 = ( rato(ir,itabs_t) -  rr(irt-1) ) / (rr(irt) - rr(irt-1))
          p2 = 1 - p1
     ...
     &                 lvval(it,l), popatv(it,l), l = 1,nlat(it))
        ...
        if( .not. nonexc ) write(3,140) necrantage, lecrantage, 
     &                               ecrantage(1:nspin)
      endif
      if( icheck > 1 ) then
        write(3,150)
...
! Calcul de la configuration electronique de l'atome excite
      ...
      subroutine screendef(ecrantage,force_ecr,icheck,itabs,
     &      lecrantage,lqnexc,lseuil,lvval,mpirank,n_orbexc,necrantage,
     &      nlat,nlatm,nnlm,nqnexc,
     &      nseuil,nspin,ntype,nvval,popexc,popval,Z)
...
! On ajoute l'ecrantage
 ...
 1010   continue
  ...
  103 format(///'   n_orb < nnlm ',//
     &          ' Increase nnlm in routine lecture.f !')
...
!***********************************************************************
      ...
      subroutine extract_Epsii(Core_resolved,Delta_Epsii,Delta_Eseuil,
     &        Epsii,Epsii_moy,icheck,nbseuil,ninit1,ninitl,ninitlr)

      use declarations
      implicit none

      integer:: i, icheck, l, nbseuil, ninit1, ninitl, ninitlr

      character(len=132) mot

      logical:: Core_resolved, Found

      real(kind=db):: Delta, Delta_Epsii, Delta_Eseuil, E, E_up, E_dn,
     &                Ep_moy, Epsii_moy
...
! Anciennes versions
      ...
      if( Core_resolved ) then
        if( ninit1 /= ninitl ) then
          Epsii_moy = sum( Epsii(ninit1+1:ninitl) )
     &              / ( ninitl - ninit1 ) 
...
!***********************************************************************
      ...
      if( tddft ) then
        do l = 1,1000000
          ...
          if( mot(2:12) == 'Cycle TDDFT'
     &                 .or. mot(2:12) == 'Cycle Tddft') exit
        end do
        goto 1005
 1000   call write_error
        do iprint = 3,9,3
          write(iprint,110)
        end do
        stop
 1005   continue
      ...
      boucle_je: do je = 1,100000
        do l = 1,10000
          read(1,'(A)',err=1010,end=1010) mot
          if( mot(2:12) == 'Cycle TDDFT'
     &                 .or. mot(2:12) == 'Cycle Tddft'
     &                 .or. mot(2:15) == 'Absorbing atom' ) goto 1010
 ...
 1010 continue
      ...
      return
  110 format(//' Error in the the indata file:',//
     &         ' An exctraction of tensors calculated using TDDFT is',
     &         ' asked.',/' The corresponding file does not contain ',
     &         ' TDDFT outputs !!')
      end

!***********************************************************************

      subroutine extract_energ(Energ_s,Eseuil,multi_run,nbbseuil,
     &                      nbseuil,nenerg_s,nom_fich_extract,tddft)
     ...
     &           .or. mot(2:12) == 'Cycle Tddft' ) exit
      ...
      if( energ_s(nenerg_s) > Eseuil(min(nbbseuil,nbseuil)) 
     &   .and. Eseuil(nbbseuil) > energ_s(nenerg_s) - energ_s(1) ) then
        do ie = 1,nenerg_s
...
! Charge et population de chaque atome de la maille
      ...
      subroutine pop_group(chargatg,Charge_free,Chargm,Flapw,icheck,
     &      itype,mpirank,ngroup,nlat,nlatm,nspin,ntype,numat,popatc,
     &      popats)
...
! Allocation des tableaux itypepr et popatm pour l'excite
      ...
      subroutine Pop_mod(chargat,chargm,Flapw,iabsorbeur,icheck,igreq,
     &      itabs,iprabs,itype,itypepr,lqnexc,lvval,
...
! le sousprogramme agregat
      ...
      subroutine natomp_cal(angxyz,axyz,Base_ortho,chargat,d_ecrant,
     &    dcosxyz,dpos,Flapw,iabsorbeur,iabsfirst,icheck,igreq,itabs,
     ...
     &    self_cons,Taux,Taux_oc,Test_dist_min)      
      ...
      integer:: iabsfirst, itabs, n_atom_proto, mpirank, multrmax,
     &          natomeq_coh, natomeq_s, natomp, ngroup
      ...
      logical base_ortho, flapw, matper, noncentre, Proto_all, 
     &        self_cons, Taux
      logical, dimension(n_atom_proto):: ipr_ok

      real(kind=db):: r_self, rsorte_s, rmax, rpotmax
      real(kind=db), dimension(3):: angxyz, axyz, dcosxyz, dpos, ps, v
          ...
          do iz = -nzmaille,nzmaille
            ...
            do igr = 1,ngroup
              if( ngroup_pdb > 0 ) then
                if( Kgroup(iabsorbeur) /= Kgroup(igr) .and.
     &            Kgroup(iabsorbeur) /= 0 .and. Kgroup(igr) /= 0 ) cycle
              endif 
      ...
      if( .not. flapw .and. matper ) then
        ...
        call reduc_natomp(axyz,Base_ortho,chargat,d_ecrant,dcosxyz,
     &          deccent,dpos,iabsorbeur,icheck,igreq,itabs,itype,Kgroup,
     &          matper,mpirank,n_atom_proto,natomp,natomr,ngreq,ngroup,
     &          ngroup_pdb,noncentre,posn,rmax,rsortm)
        natomp = natomr
      endif
...
! Test sur les atomes trop proches
        ...
        do igr1 = 1,ngroup
          do igr2 = igr1+1,ngroup
            if( ngroup_pdb > 0 ) then
              if( Kgroup(igr1) /= Kgroup(igr2) .and.
     &            Kgroup(igr1) /= 0 .and. Kgroup(igr2) /= 0 ) cycle
            endif 
            ps(1:3) = posg(1:3,igr1) - posg(1:3,igr2)
            r = vnorme(Base_ortho,dcosxyz,ps)
            if( r > Test_dist_min ) cycle
            if( Taux .and. r < eps10 ) then 
  ...
  140 format(/' This can come from:'/
     ...
     &2x,'- Two atoms too close,'/,
     &2x,'- When using Spgroup keyword, the position of an atom',
     &    ' standing on',/
     &2x,'  a symmetry plane or on a rotation axis must follow',
     &   ' precisely the convenient',/
     &2x,'  ratio between x, y and z of the site (for example',
     &   ' (h,2h,0) ),'/,
...
!*********************************************************************
      ...
      subroutine reduc_natomp(axyz,Base_ortho,chargat,d_ecrant,dcosxyz,
     &          deccent,dpos,iabsorbeur,icheck,igreq,itabs,itype,Kgroup,
     &          matper,mpirank,n_atom_proto,natomp,natomr,ngreq,ngroup,
     &          ngroup_pdb,noncentre,posn,rmax,rsortm)
      ...
      integer:: ia, iaabs, iaabsfirst, iabsorbeur, icheck, igr, ipr, 
     &          itabs, mpirank, n_atom_proto, na, natomp,
     &          natomq, natomr, ngroup, ngroup_pdb
      ...
      logical:: Base_ortho, matper, noncentre
     ...
     &                rmax, rsortm           
      ...
      matper = .true.
      call clust(axyz,Base_ortho,dcosxyz,deccent,dista,dpos,iaabs,
     &           iaabsfirst,iabsorbeur,igroup,itabs,itype,itypep,Kgroup,
     &           matper,mpirank,1,natomp,ngroup,ngroup_pdb,
     &           noncentre,.false.,pos,posn,rmax)
...
!*********************************************************************
      ...
      subroutine clust(axyz,Base_ortho,dcosxyz,deccent,dista,dpos,iaabs,
     &            iaabsfirst,iabsorbeur,igroup,itab,itype,itypep,Kgroup,
     &            matper,mpirank,multi_run,natomp,ngroup,ngroup_pdb,
     &            noncentre,One_run,pos,posn,rmax)

      use declarations
      implicit none

      integer:: ia, ia1, ia2, iaabs, iaabsfirst, iabsorbeur, igr, igr12, 
     &  ipr, itab, ity12, ix, iy, iz, mpirank, multi_run,
     &  natomp, ngroup, ngroup_pdb, nxmaille, nymaille, nzmaille
      integer, dimension(natomp):: igroup, itypep
      integer, dimension(ngroup):: itype
      integer, dimension(ngroup_pdb):: Kgroup

      logical:: Base_ortho, Matper, Noncentre, One_run
...
! Elaboration de l'agregat.
      ...
      subroutine agregat(angxyz,Atom_with_axe,Atom_nonsph,
     &      Axe_atom_clu,Axe_atom_gr,Axe_atom_grn,axyz,Base_hexa,
     &      Base_ortho,chargat,chargm,Cubmat,dcosxyz,dista,dpos,Flapw,
     &      Hubbard,iaabs,iaabsfirst,iabsorbeur,iaproto,iapot,icheck,
     &      igreq,igroup,igrpt_nomag,iopsymc,iopsymr,itab,itype,
     ...
     &                 PointSubGroup_name, PointSubGroup_Sch
      ...
      integer:: igrpt, igrpt_nomag, igrpt_sg, igrpt_sg_cal, multi_run
      integer, dimension(natomp):: iaproto, igroup, itypep
     ...
     &        hubbard, Magnetic, matper, noncentre, nonexc_g, 
     &        One_run, PointGroup_Auto, self_nonexc, spinorbite, sym_4,
     &        sym_cubic, symmol
      logical, dimension(0:ngroup_m):: Atom_with_axe

      real(kind=db), dimension(3):: angxyz, Axe_spin, Axe_spin_0, axyz, 
     &                              dcosxyz, dpos, pa, pb, v
      real(kind=db), dimension(3,3):: Cubmat, Orthmat, Orthmati,
     &       orthmatt, rot_un, rotmat, Rot_int, rot_so, rot_tem, rotspin
      real(kind=db), dimension(natomp):: dista
     ...
     &            noncentre,One_run,pos,posn,rmax)
      ...
      call cal_iaeqrmt(iaabs,iaproto,iapot,igreq,igroup,
     &       n_atom_proto,natomp,neqm,ngreq,nonexc_g,self_nonexc)
...
! Determination des symetries du groupe ponctuel (iopsymc)
      ...
      if( PointGroup_Auto ) then
        ...
        do i = 1,2
          ...
          call sym_cluster(Atom_with_axe,Axe_atom_clu,
     &           Base_ortho,dcosxyz,iaabs,igroup,iopsymc,itype,itypep,
     &           natomp,ngroup,ngroup_m,nlat,nlatm,nspin,ntype,numat,
     &           popats,pos)
...
! On prend un sous-groupe.
      ...
      if( PointGroup_Auto ) then
        ...
        igrpt_sg = igrpt_sg_cal(hubbard,igrpt,igrpt_nomag,
     &                          Spinorbite)
      else
        igrpt_sg = igrpt
      endif
...
! On met une charge opposee sur la sphere a dista(natomp) du centre
  ...
  210 format(/' Cluster: atom positions in order, in the internal R2',
     & ' bases',/
...
! reelles
      ...
      integer function igrpt_sg_cal(Hubbard,igrpt,igrpt_nomag,
     &                              Spinorbite)
...
! Sous-programme calculant la symetrie ponctuelle de l'agregat
      ...
      subroutine sym_cluster(Atom_with_axe,Axe_atom_clu,
     &           Base_ortho,dcosxyz,iaabs,igroup,iopsym,itype,itypep,
     &           natomp,ngroup,ngroup_m,nlat,nlatm,nspin,ntype,numat,
     &           popats,pos)
      ...
      real(kind=db), dimension(3):: Axe_atom_c, Axe_atom_s, dcosxyz, ps,
     &                           pt, vspin, vspini, wspin, wspini
...
!     &                                               cycle boucle_sym
              ...
              if( .not. ( Atom_with_axe(igra) 
     &            .or. Atom_with_axe(igrb) ) ) then
                iok = 1
                exit
              endif

              if( ( Atom_with_axe(igra) .and. .not. Atom_with_axe(igrb))
     &        .or.(.not. Atom_with_axe(igra) .and. Atom_with_axe(igrb)))
     &                                              cycle boucle_exter
...
! Sousprogramme calculant la forme des tenseurs
      ...
      subroutine Tensor_shape(Atom_with_axe,Atom_nonsph,
     &             Axe_atom_clu,Base_ortho,dcosxyz,Dipmag,E1E1,E1E2,
     &             E1E2e,E1E3,E3E3,Green,iaabs,icheck,igroup,iopsymc,
     ...
     &   Atom_nonsph, Base_ortho, Dipmag, E1E1, E1E2, E1E2e, E1E3,
     &   E3E3, Green, magnet, Magnetic, Octupole, Quadrupole, 
     &   Spinorbite, State_all 
      logical, dimension(0:ngroup_m):: Atom_with_axe
...
! Evaluation de la symetrie de l'atome absorbeur
      ...
      if( nspin == 1 .and. sum( abs( pos(:,iaabs) ) ) < eps10 ) then
        ...
        if( spinorbite ) then  
          call point_group_atom(Atom_comp,Atom_mag,Atom_with_axe,
     &      Atom_nonsph,Axe_atom_clu,Base_ortho,dcosxyz,
     &      iaabs,iaabs,icheck,igroup,igroup(iaabs),
     &      igrpt,iopsym_abs,iopsymr,itype,itypep,magnetic,mpirank,
     &      natomp,ngroup,ngroup_m,nlat,nlatm,nspin,ntype,
...
! On prend la symetrie complete (iopsymc)
     ...
     &      Atom_nonsph,Axe_atom_clu,Base_ortho,dcosxyz,
     &      iaabs,iaabs,icheck,igroup,igroup(iaabs),
     &      igrpt,iopsym_abs,iopsymc,itype,itypep,magnetic,mpirank,
     &      natomp,ngroup,ngroup_m,nlat,nlatm,nspin,ntype,
     &      numat,popats,pos,ps,rot_atom_abs,spinorbite)
        endif
      endif
! La rotation est calcule avec la symetrie utilisee (iopsymr)
     ...
     &    Atom_nonsph,Axe_atom_clu,Base_ortho,dcosxyz,
     &    iaabs,iaabs,icheck,igroup,igroup(iaabs),
     &    igrpt,iopsymt,iopsymr,itype,itypep,magnetic,mpirank,
     &    natomp,ngroup,ngroup_m,nlat,nlatm,nspin,ntype,
     &    numat,popats,pos,ps,rot_atom_abs,spinorbite)
...
! Calcul des tenseurs
        if( E1E1 .or. dipmag ) call tensdd(irotiops,magnet,msymdd,
     &                                                     msymddi)
        if( E1E2e ) call tensdq(irotiops,magnet,msymdq,msymdqi)
        if( Quadrupole ) call tensqq(irotiops,magnet,msymqq,msymqqi)
              ...
              do hs = 1,3
                do js = 1,3
                  ...
                  do he = 1,3
                    do je = 1,3
                      jhe = 3 * ( je - 1 ) + he
                      write(3,167) je, he, ( ( msymoo(i,jhe,k,jhs),
     &                        msymooi(i,jhe,k,jhs), i = 1,3), k =1,3)
                    end do
                  end do
                end do
              end do
              ...
              do he = 3,1,-1
                ...
                do ks = 3,1,-1
                  do js = 3,1,-1
                    do hs = 3,1,-1
                      jhs = 3*(js - 1) + hs
                      if( abs(msymoo(ke,jhe,ks,jhs)) == i .or.
     &                 abs(msymooi(ke,jhe,ks,jhs)) == i ) exit boucle_oo
                    end do
                  end do
                end do
              end do
...
! Calcul du tenseur E3E3
     ...
     &  iis, is, isg, isgi, iordresym, isgmag, iss, j, j1, j2, j3, 
     &  je, jhe, jhs, jj, jje, jje_s, jjhe, jjhs,
     &  jjs, jjs_s, js, ke, kk, kke, kke_s, kkk, kkkmax, kks, kks_s,
     &  ks, n, n_oo
            ...
            do ks = 1,3
              do js = 1,3
                do hs = 1,3
                  ...
                  if( msymoo(ke,jhe,ks,jhs) /= 0 .or. 
     &                msymooi(ke,jhe,ks,jhs) /= 0 ) cycle
                  ...
                  do iss = 2,min(48,nopsm)
                    ...
                    if( ( isg > 0 .and.
     &                        msymoo(kke,jjhe,kks,jjhs) == - kkk ) .or.
     &                  ( isg < 0 .and. 
     &                        msymoo(kke,jjhe,kks,jjhs) == kkk ) )
     &                                              impos(kkk) = .true.
                    if( ( isgi > 0 .and.
     &                        msymooi(kke,jjhe,kks,jjhs) == - kkk ).or.
     &                ( isgi < 0 .and.
     &                        msymooi(kke,jjhe,kks,jjhs) == kkk ) )  
     &                                             imposi(kkk) =.true.
                    ...
                    do iie = 1,6
                      ...
                      do iis = 1,6
                        ...
                        if( .not. imposi(kkk) .and. isgmag /= 0 .and.
     &                    (kke /= kks .or. jje /= jjs .or. hhe /= hhs) )
     &                    then
      ...
      do kkk = 1,kkkmax
        ...
        if( impos(kkk) .and. imposi(kkk) ) then
          ...
          where( abs(msymooi) > kk ) msymooi = msymooi
     &                                       - abs(msymooi) / msymooi
          kk = kk - 1
        endif
      end do
        ...
        do ke = 1,3
          do jhe = 1,n_oo
            do ks = 1,3
              do jhs = 1,n_oo
                if( abs(msymoo(ke,jhe,ks,jhs)) == i .or. 
     &              abs(msymooi(ke,jhe,ks,jhs)) == i ) cycle boucle_i
              end do
            end do
          end do
        end do
        where( abs(msymoo) > i ) msymoo = msymoo - abs(msymoo) / msymoo
        where( abs(msymooi) > i ) msymooi = msymooi
     &                                       - abs(msymooi) / msymooi
      end do boucle_i
...
!*********************************************************************
      ...
      subroutine cal_iaeqrmt(iaabs,iaproto,iapot,igreq,igroup,
     &       n_atom_proto,natomp,neqm,ngreq,nonexc_g,self_nonexc)
...
! Calcul de l'absorption avant seuil
      ...
      real(kind=db) function fpp_cal(Eseuil,Full_self_abs,icheck,
     ...
     &        self_abs,Taux_ipr,Volume_maille)
      ...
      integer icheck, ipr, n_atom_proto, nbseuil, ntype, Z
      integer, dimension(0:ntype):: numat
      integer, dimension(0:n_atom_proto):: itypepr, ngreq

      logical Full_self_abs, self_abs
      
      real(kind=db):: Conv_mbarn_nelec, Ea, fpp_avantseuil,
     &                Volume_maille, x
      real(kind=db), dimension(nbseuil):: Eseuil
...
! Conversion en coefficient d'absorption lineaire en micrometre
  ...
  310 format(/' Absorption before the edge :',/
     &        ' Site  Z      fpp (per atom in nbr of electron)')
  320 format(2i4,1p,e13.5)
  330 format(/' Total :',1p,e13.5,' micrometer')
      end
...
! Sousprogramme effectuant certaines preparations pour le DAFS
      ...
      subroutine prepdafs(Angle_or,angpoldafs,angxyz,Axe_atom_gr,axyz,
     &          Base_spin,Bormann,Dafs_bio,Eseuil,hkl_dafs,icheck,
     &          igreq,iprabs,isigpi,itabs,itypepr,lvval,Magnetic,
     ...
     &  nbseuil, neqm, ngrm, ngroup, ngroup_m, ngroup_taux, ngroup_temp,
     ...
     &    phd_f0_cjg, phd_fan_cjg, phd_fan, phd_fmag   
      ...
      logical:: Base_spin, Bormann, Dafs_bio, Debye, Magnetic, Taux,
     &          Temperature

      real(kind=sg):: getf0, s
      real(kind=db):: Angle, arg, cos_pe, cos_ps, cosang, cosb, cosp,
     &  Deb, Delta_2, deltak_A, detmat, deuxpi, dhkl, dp, dpdeg, DW,
     &  Emc2, f_no_res_mag, f_no_res_mom, fac, fmo, fms, konde, pp, psi,
     &  qkn, rad, rap_lsur2s, sin_pe, sin_ps, sinb, sinp, Temp, Tempt,
     &  Thetabragg, vol, wn, wsn
      real(kind=db), dimension(3):: ang, angxyz, axyz, cosdir, hklred,
     &   qi, qj, qk, v, Vec_orig, vpie, vpe, vps, vpis, vsig, vx, vy,
     &   vz, w, we, ws, wx, wy, wz
      ...
      real(kind=db), dimension(3,3):: Mat, Mat_or, Mat_ori, Orthmat,
     &                                Orthmati, Rot_int
      real(kind=db), dimension(3,ngroup):: posn
      real(kind=db), dimension(3,ngroup_m):: Axe_atom_g, Axe_atom_gr
      real(kind=db), dimension(3,npldafs):: angpoldafs, vecdafsem,
     &                                      vecdafssm
...
! Quand angpoldafs(3,ipl) = 10000, polarisation et veconde sont imposes 
        if( angpoldafs(3,ipl) > 9999._db ) then
          ...
          poldafse(:,:,1) = poldafsem(:,:)
          poldafss(:,:,1) = poldafssm(:,:)
          vecdafse(:,:,1) = vecdafsem(:,:)
          vecdafss(:,:,1) = vecdafssm(:,:)
...
! Reflection (-Q)
            ...
            do ip = 1,nphi_dafs(ipl)
              ...
              select case( isigpi(ipl,1) )
                case(3)
                  poldafse(:,ipl,ip) = cmplx( vsig(:), vpie(:), db)
     &                               / sqrt(2._db)
                case(4)
                  poldafse(:,ipl,ip) = cmplx( vsig(:),-vpie(:), db)
     &                               / sqrt(2._db)
              ...
              select case( isigpi(ipl,2) )
                case(3)
                  poldafss(:,ipl,ip) = cmplx( vsig(:), vpis(:), db)
     &                               / sqrt(2._db)
                case(4)
                  poldafss(:,ipl,ip) = cmplx( vsig(:),-vpis(:), db)
     &                               / sqrt(2._db)
...
! ThetaBragg is here the scattering angle
            dp = dot_product(v,w)
            if( abs( dp - 1._db ) < eps10 ) then
              Thetabragg = 0._db
            else          
              Thetabragg = 0.5_db * acos( dot_product(v,w) )
            endif
...
! Temp_coef = 8*pi^2 * <u>^2 est en Angtroem^2
          ...
          do igr = 1,ngreq(ipr)
            ...
            if( Debye .or. Temperature) Bragg(igr,ipl)
     &                                  = Bragg(igr,ipl) * Deb
...
! calcule dans le programme !
        if( n /= numat(itabs) ) then 
          call fprime(n,eseuil(nbseuil),fpp(ipr),fp(ipr))
        else
        ...
        do ipl = 1,npldafs
          ...
          if( Dafs_bio ) then
            ...
            phd_fan_cjg(ipr,ipl) = conjg( phd(ipr,ipl) )
     &                       * cmplx(fp(ipr), fpp(ipr), db)
          endif
          ...
          if( Magnetic ) then
            ...
            if( abs(f_ms(ipr,ipl)) > eps10 ) then
              ...
              do igr = 1,ngreq(ipr)
                jgr = igreq(ipr,igr)
! cosang is to take into acount the spin orientation
                cosang = sum( Axe_Atom_gr(:,jgr) * Axe_Atom_gr(:,kgr) )
...
! Bragg est deja en Green -.
                phd_fms(ipr,igr,ipl) = - img * cosang
     &                              * Bragg(igr,ipl) * f_ms(ipr,ipl)
                phd_fmo(ipr,igr,ipl) = - img * cosang
     &                              * Bragg(igr,ipl) * f_mo(ipr,ipl)
              end do
            endif
      ...
      if( icheck > 1 .and. magnetic ) then
        write(3,'(/A)') ' ipr ipl    phd_fms(ipr,ipl,1..ngr)'  
        do ipr = 1,n_atom_proto
          do ipl = 1,npldafs
            if( abs(f_ms(ipr,ipl)) < eps10 ) cycle
            write(3,'(2i4,1p,50e13.5)') ipr, ipl, 
     &                                  phd_fms(ipr,1:ngreq(ipr),ipl)  
          end do
        end do
      endif
...
! Multplication par epsilon_e * espilon_s
      do ipl = npldafs,1,-1
        ph = sum( phd_f0(1:n_atom_proto,ipl) )
     &     + sum( phd_fan(1:n_atom_proto,ipl) )
        if( Dafs_bio ) ph_cjg = sum( phd_f0_cjg(1:n_atom_proto,ipl) )
     &     + sum( phd_fan_cjg(1:n_atom_proto,ipl) )  
...
! Voir aussi appendix de G. T. Trammell, PR 92, 1387 (1953)
            ...
            do ipr = 1,n_atom_proto
              ...
              do igr = 1,ngreq(ipr)
                ...
                if( nphi_dafs(ipl) > 1 ) phdf0t(ipl,ip) = phdf0t(ipl,ip)
            ...
            do ip = 1,nphi_dafs(ipl)
              ...
              if( Dafs_bio ) then
                ...
                if( ip == 1 .or. ip == 2 ) then
                  write(3,215) hkl_dafs(:,ipl), mot4, real(pe(:)), 
     &            real(ps(:)), we(:), ws(:)
                else
                  write(3,215) -hkl_dafs(:,ipl), mot4, real(pe(:)), 
     &            real(ps(:)), we(:), ws(:)
                ...
                write(3,220)  ( ip - 1 ) * dpdeg, real(pe(:)), 
     &            aimag(pe(:)), real(ps(:)), aimag(ps(:)), we(:), ws(:)
              endif
            end do
          end do
        endif
...
! On passe en base R1
      ...
      if( icheck > 1 ) then
        ...
        do ipl = 1,npldafs
          ...
          do ip = 1,nphi_dafs(ipl)
            write(3,*)
            do i = 1,3
             write(3,380) poldafse(i,ipl,ip), vecdafse(i,ipl,ip),
     &                    poldafss(i,ipl,ip), vecdafss(i,ipl,ip)
            end do
          end do
        end do
      endif
  ...
  200 format(/'  (h, k, l)   pol',11x,'Poldafse',20x,
     &    'Poldafss',20x,'Vecdafse',20x,'Vecdafss')
  210 format(/' (h,k,l) = (',3i3,'),  isigpi =',2i3,/' angle',
     &8x,'real(Poldafse)',13x,'imag(Poldafse)',11x,'real(Poldafss)',12x,
     &    'imag(Poldafse)',15x,'Vecdafse',15x,'Vecdafss')
  ...
  370 format(/'  Polarization and wave vectors in the internal basis',
     & ' R4 ( orthogonal basis, z along spin direction )',//
     &        '         pol            vec           pls',12x,'vos')
  375 format(/'  Polarization and wave vectors in the internal basis',
...
!***********************************************************************
      ...
      subroutine col_dafs_name(angpoldafs,Bormann,Full_self_abs,
     ...
     &       nomabs,npldafs,Self_abs)
      ...
      integer:: i, icol, index, ipldafs, ipr, j, k, Length_word,
     &          mpirank, ncolm, ncolr, ncolt, npldafs
...
!***********************************************************************
      ...
      subroutine get_fmag(deltak,fmo,fms,ipr,it,lvval,
...
! 1 uam = 1.66053*10**(-27)kg
      x = - 300 * (q*hbar)**2 * FD
     &  / ( Mass_atom(Z) * 1.66053 * k_Boltzmann * TD(Z) )
...
! Sousprogramme evaluant les orbitales sondees par les regles de 
! selection des transitions

      subroutine etafin(Ylm_comp,icheck,iopsymr,irep_util,
     ...
     &          State_all,Sym_cubic)
      ...
      logical:: lmoins1, lplus1, spinorbite,
     &        state_all, sym_cubic, Ylm_comp
...
! Calcul du (l,m) = (loperat,moperat) de la transition
     ...
     &                                                 cycle boucle_ll
      ...
      if( state_all ) then
        call irep_util_all(icheck,iopsymr,irep_util,karact,nb_rep,
     &                     ngrph,nspino,Spinorbite)
      else
        call cal_irep(icheck,iopsymr,irep_util,iselec,karact,lselec,
     &                mselec,nb_rep,ngrph,nlfm,nselec,nspino,Spinorbite)
      endif
...
!*********************************************************************
      ...
      subroutine cal_irep(icheck,iopsymr,irep_util,iselec,karact,lselec,
     &                mselec,nb_rep,ngrph,nlfm,nselec,nspino,spinorbite) 
     ...
     &                 lass(lf)
      ...
      if( nb_rep == 1 ) then
        ...
        boucle_l: do lf = 1,nselec
          ...
          ispg = nspino + 1 - ispf
          do igrph = 1,ngrph
            if( irep_util(igrph,ispf) == irepo(lf) .and.
     &          irep_util(igrph,ispg) == irepo(lg) ) cycle boucle_l
          end do
          ngrph = ngrph + 1
...
! la densite totale, il faut avoir toutes les representations.
      ...
      subroutine irep_util_all(icheck,iopsymr,irep_util,karact,nb_rep,
     &                         ngrph,nspino,spinorbite)
...
! Sousprogramme calculant les valeurs des vecteurs polarisation et onde
      ...
      subroutine polond(Base_spin,Dipmag,Green_plus,icheck,
     ...
     &      pdpolar,pol,polar,Quadrupole,Rot_int,veconde,vec,Xan_atom)
      ...
      logical:: Base_spin, dipmag, green_plus, moyenne,
     &          octupole, Polarise, Quadrupole, Xan_atom
...
! On se place dans la base cristallographique
     ...
     &       .and. ( Quadrupole .or. Octupole .or. Dipmag ) ) then
            j = j + 1
            nomab(j:j) = ','
            call trnom(j,kpol,Length_word,nomab)
          endif
      ...
      if( nplr > 1 .and. ( abs(pp1) > eps4 .or. abs(pp2) > eps4) ) then
        pdp(1:nplr,1) = pdp(1:nplr,1) / pp1
        if( Quadrupole .or. Octupole ) pdp(1:nplr,2) = pdp(1:nplr,2)/pp2
        Moyenne = .true.
        jpl = jpl + 1
        nomabs(jpl) = '    <xanes>  '
      else
...
!***********************************************************************
      ...
      function natome_cal(igrpt_nomag,iopsymr,mpirank,natomeq,
     &                    natomp,pos)
...
! Evaluation de leur groupe ponctuel dans cet agregat.
      ...
      subroutine Atom_selec(adimp,Atom_axe,Atom_with_axe,
     &         Atom_nonsph,Atom_occ_mat,Axe_atom_clu,
     &         Base_ortho,dcosxyz,dista,distai,
     &         Full_atom,Green,hubbard,i_self,ia_eq,ia_eq_inv,
     &         ia_rep,iaabs,iaabsi,iaproto,iaprotoi,icheck,igreq,
     ...
     &        Base_ortho, Full_atom, Green, Hubbard, magnetic, overad, 
     &        spinorbite, Ylm_comp, Ylm_comp_inp
     ...
     &      cycle
      ...
      if( Atom_occ_mat .and. hubbard .and. i_self == 1 ) then
        ...
        do iapr = n_atom_0_self,n_atom_ind_self
          ...
          do m = -l,l
            do mp = -l,l
              if( m == mp ) V_hubb(m,mp,:,iapr) = 0.5_db * V_hubbard(it) 
              V_hubb(m,mp,:,iapr) = V_hubb(m,mp,:,iapr)
     &                         - V_hubbard(it) * occ_hubb_e(m,mp,:,igr)
            end do
          end do
        end do
      end if
...
! Evaluation de la symetrie locale
      do ia = 1,natome
        ...
        ps(:) = posi(:,ia)
        call point_group_atom(Atom_comp(ia),Atom_mag(ia),Atom_with_axe,
     &      Atom_nonsph,Axe_atom_clu,Base_ortho,dcosxyz,
     &      iaabs,iatomp(ia),icheck,igroup,igroupi(ia),
     &      igrpt(ia),iopsyma,iopsymr,itype,itypep,magnetic,mpirank,
     &      natomp,ngroup,ngroup_m,nlat,nlatm,nspin,ntype,
...
! Parametres du maillage
     ...
     &            iatomp(ia), posi(1:3,ia)*bohr, igrpt(ia),
     &            ptgrname_int(igrpt(ia)), Ylm_comp, Atom_axe(ia), 
     &            Atom_mag(ia), Axe_atom_clui(:,ia)
          else
            write(3,190) ia, numat(it), it, igroupi(ia), iaprotoi(ia),
     &            iatomp(ia), posi(1:3,ia)*bohr, igrpt(ia), 
     &            ptgrname_int(igrpt(ia)), Ylm_comp, Atom_axe(ia),
...
! un point entre les atomes.
      ...
      if( .not. Green ) then
        do ia1 = 1,natome
          ...
          do ia2 = ia1+1,natome
            ...
            if( dist < rmt(it1) + rmt(it2) - adimp
     &                              .and. mpirank == 0) then
              call write_error
              do ipr = 3,9,3
                write(ipr,280) ia1, numat(itypei(ia1)), igroupi(ia1),
     &                 ia2, numat(itypei(ia2)), igroupi(ia2), dist*bohr,
  ...
  170 format(/'  ia   Z  it  igr ipr iap     posx      posy',
     &'      posz   igrpt PtGrName Comp  Axe  Mag',9x,'Axe_atom')
  180 format(/'  ia   Z  it  igr ipr iap     posx      posy',
     &'      posz   igrpt PtGrName Comp  Axe  Mag')
  ...
  280 format(//' When using the FDM mode, the minimum distance between',
     &  ' the atoms is the sum',/' of their Rmt radius plus',
     &  ' the interpoint distance.',//
     &  '   The atoms',i4,' (Z =',i3,', igr =',i4,') and',i4,
     &  ' (Z =',i3,', igr =',i4,') are too close.',//
...
!***********************************************************************
      ...
      subroutine point_group_atom(Ylm_comp,Atom_mag,Atom_with_axe,
     &        Atom_nonsph,Axe_atom_clu,Base_ortho,dcosxyz,
     &        iaabs,iap,icheck,igroup,igra,
     &        igrpt,iopsym,iopsymr,itype,itypep,magnetic,mpirank,natomp,
     &        ngroup,ngroup_m,nlat,nlatm,nspin,ntype,numat,
     &        popats,pos,posi,rot_atom,spinorbite)
      ...
      logical Ylm_comp, Atom_comp_cal, Atom_mag, Atom_mag_cal,
     &        Atom_nonsph, Base_ortho, Magnetic, spinorbite
      logical, dimension(0:ngroup_m):: Atom_with_axe
...
! Determination des symmetries du groupe ponctuel (iopsymc)
        do i = 1,2
          ...
          call sym_cluster(Atom_with_axe,Axe_atom_clut,
     &           Base_ortho,dcosxyz,iaabs,igroup,iopsym,itype,itypep,
     &           natomp,ngroup,ngroup_m,nlat,nlatm,nspin,ntype,numat,
     &           popats,p)
...
! de l'agregat.
          call iop_rot(icheck,irotiops,transpose(rot_atom))
      ...
      if( Spinorbite ) then
        ...
        call iop_rot(icheck,irotiops,transpose(rot_tem))
...
! Calcul du nombre de point du maillage.
      ...
      subroutine nbpoint(Adimp,Base_hexa,Base_ortho,D_max_pot,dcosxyz,
     &             Green,iaabs,igrpt_nomag,iopsymr,iord,Moy_loc,
...
! Elaboration du maillage.
      ...
      subroutine reseau(Adimp,Base_hexa,Base_ortho,D_max_pot,dcosxyz,
     &             Green,iaabs,icheck,
      ...
      do ix = -nx,nx
        do iy = -nx,nx
          ...
          do iz = -nx,nx
            ...
            w(:) = v(:)
            call posequiv(mpirank,w,iopsymr,isym,igrpt_nomag)
            if( abs(v(1)-w(1)) > epspos .or. abs(v(2)-w(2)) > epspos
     &                           .or. abs(v(3)-w(3)) > epspos ) cycle
            if( Green .and. .not. Moy_loc ) then
! Pour la moyenne du potentiel, on ne prend pas les points loin de tout
! atome
              do ia = 1,natomp
...
! Routine calculant les coefficients du laplacien.
      ...
      subroutine laplac(Adimp,Base_hexa,cgrad,clapl,icheck,
...
! centre de l'atome correspondant.
      ...
      subroutine bordure(Base_ortho,dcosxyz,Green,icheck,iopsymr,iord,
     &           iscratch,ivois,mpirank,natome,nbm,nbtm,nim,
...
! Calcul de l'energie du niveau de coeur initial.
      ...
      subroutine Energseuil(Core_resolved,Delta_Epsii,Delta_Eseuil,
     &         Epsii,Epsii_moy,Eseuil,icheck,is_g,
     &         itabs_nonexc,lseuil,m_g,mpirank,nbseuil,ninit1,ninitl,
     &         ninitlr,nr,nrm,nseuil,nspine,ntype,
     &         numat,psii,rato,Rmtg,
     &         Rmtsd,V_abs_i,V_intmax,V0bde)
    
      use declarations
      implicit none

      integer icheck, initl, ipr, isol, isp, itabs_nonexc, jsp, lseuil,
     &  m, mpirank, nbseuil, ninit1, ninitl, ninitlr, nm1, nm2, nr, 
     &  nrm, nseuil, nsol, nspin, nspine, nspino, ntype, numat

      parameter(nspin = 2, nspino = 2)

      logical:: Core_resolved, Relativiste, Spinorbite

      integer, dimension(ninitl):: is_g
      integer, dimension(ninitl,2):: m_g

      real(kind=db):: Delta, Delta_Epsii, Delta_Eseuil, Ep_moy,
     &  Epsii_moy, j, mj, psiHpsi, Rmtg, Rmtsd, V_intmax
      ...
      real(kind=db), dimension(nr,nspin):: V_abs
      real(kind=db), dimension(0:nrm,0:ntype):: rato
      ...
      do isp = 1,nspin
        jsp = min(isp,nspine)
        V0bd(isp) = V0bde(jsp)
        V_abs(1:nr,isp) = V_abs_i(1:nr,jsp)
      end do
...
! spinorbite
      ...
      do initl = 1,ninitl
        ...
        Epsi(initl) = - psiHpsi(.true.,icheck,isol,lseuil,m,
     &             nseuil,nr,nr,nsol,nspin,
     &             nspino,numat,V0bd,V_abs,psi,r,Relativiste,Rmtg,
     &             Rmtsd,Spinorbite,V_intmax)
      end do
     ...
     &              / ( ninitl - ninit1 ) 
      ...
      do ipr = 3,6,3
        ...
        if( ( ipr == 3 .and. icheck > 0 ) .or.
     &      ( ipr == 6 .and. Core_resolved .and. mpirank == 0 ) ) then
          if( ipr == 3 ) write(ipr,115)
          if( ipr == 3 .and. ninit1 /= ninitl ) write(ipr,120)
     &                                                     Delta * rydb
          mj = - j - 1
          do initl = 1,ninitl
            mj = mj + 1
            ...
            if( Delta_Epsii < 100000._db .and. nbseuil > 1 ) then
              write(ipr,130) dEpsii(initl)*rydb, Epsi(initl)*rydb, 
     &                       initl, j, mj
            else
        ...
        if( .not. Core_resolved .and. ( ( ipr == 3 .and. icheck > 0)
     &                   .or. ( ipr == 6 .and. mpirank == 0 ) ) ) then
          if( ipr == 3 ) then
            if( ninitlr == 1 ) then
  ...
  120 format(4x,'Edge(1) - E_edge(2) - ( <E_KS(1)> - <E_KS(2)> ) =',
     &    f6.3,' eV',/)
  130 format(4x,'Epsii =',f10.3,' eV,  E_KS =',f10.3,  
     &       ' eV,  State',i3,', j =',f5.1,', mj =',f5.1)
  135 format(4x,'Epsii =',f10.3, 
     &       ' eV,  State',i3,', j =',f5.1,', mj =',f5.1)
...
! depend de l'energie calculee.
      ...
      function psiHpsi(Cal_psi,icheck,isol,l,m,
     &            n,nr,nrm,nsol,nspin,
     &            nspino,numat,V0bd,Vrato,psi,r,Relativiste,Rmtg,
     &            Rmtsd,Spinorbite,V_intmax)

      use declarations    
      implicit none

      integer:: icheck, ie_cycle, im, ip, ip_cycle, ir, isol, isp,  
     &  l, l2, lmax, m, m_hubb, m1, m2, n, nie_cycle, nip_cycle,
     &  nodes, nr, nr_max, nr_pos, nrm, nrmtg, nrmtsd, nsol, nspin,
     &  nspino, numat

      parameter(m_hubb = 0)

      logical:: Cal_psi, Ecomp, Full_potential, Hubb_a, Hubb_d,
     ...
     &  k_onde, php, pp, psiHpsi, Rmtg, Rmtsd, spp, td, V_intmax, vme
      ...
      real(kind=db), dimension(nrm,nspin):: g0, gm, gp, gpi, ur, V,
     &                                      Vrato
      real(kind=db), dimension(nrm,nspino):: gso
      real(kind=db), dimension(-m_hubb:m_hubb,-m_hubb:m_hubb,nspin,
     &                                       nspin):: V_hubb
      real(kind=db), dimension(:,:,:,:,:), allocatable:: urs
      ...
      call mod_V(icheck,nrm,nrmtg,nrmtsd,nspin,r,Rmtg,Rmtsd,V,
     &                 V_intmax,V0bd,Vrato)
      ...
      do ip_cycle = 1,nip_cycle 
        ...
        do ie_cycle = 1,nie_cycle  ! cycle pour le calcul de l'energie
          ...
          if( ip_cycle == 1 .and. ie_cycle == 1 ) then
            call coef_sch_rad(E,f2,g0,gm,gp,gso,nr,nspin,nspino,
     &                        numat,r,.false.,.false.,V)
            gpi(:,:) = 1 / gp(:,:)
...
! Il faut ajouter E car g0 contient V - E
              ...
              Hpsi = gm(ir,isp) * ur(ir-1,isp) +  td * ur(ir,isp)                           
     &             + gp(ir,isp) * ur(ir+1,isp)
              if( nsol > 1 .and. (ip_cycle > 1 .or. ie_cycle >1))
     &                 Hpsi = Hpsi + f_so * gso(ir,isp) * ur(ir,3-isp)
...
! Konde ne sert pas, car on ne calcule pas Tau
          ...
          allocate( us(nr,m1,m2,nspin,nspino) )
          call Sch_radial(Ecinetic,Ecomp,Eimag,f2,
     &          Full_potential,g0,gm,gpi,gso,Hubb_a,Hubb_d,icheck,konde,
     &           l,lmax,m_hubb,m1,m2,nr,nrmtg,nspin,nspino,numat,r,
     &           Radial_comp,Relativiste,Renorm,Rmtg,Spinorbite,Tau,
     &           us,urs,V_hubb)
          ...
          do ir = nrmtg,1,-1
            if( V(ir,1) < E .or. V(ir,nspin) < E ) exit
          end do
          nr_pos = ir
          ...
          if( nodes == n - l - 1 .or. E > V(nrmtg,1) ) exit
...
! Inward integration
          do ir = nr,1,-1
            if( ( V(ir,1) - E ) * r(ir)**2 < 625._db ) exit
          end do
          nr_max = ir
          ...
          do isp = 1,nspin
            ...
            do ir = nr_max-1,nr_pos+2,-1
              ...
              ur(im,isp) = - ( td * ur(ir,isp)
     &                       + gp(ir,isp) * ur(ip,isp) ) / gm(ir,isp)
            end do
      ...
      if( icheck > 2 ) then
        ...
        do ir = 1,nr
          write(3,140) r(ir)*bohr, psi(ir), ur(ir,:), V(ir,:)*Rydb           
        end do
      endif
  ...
  130 format('    rato        psi      psi_new(up)   psi_new(dn)',
     &'       Vr_up       Vr_dn')
...
! Calcul des (l,m) de chaque representation
      ...
      subroutine lmrep(Green,iaprotoi,iato,icheck,iopsym_atom,
...
! Calcul des Ylm et des distances au centre de l'atome de chaque point.
      ...
      subroutine ylmpt(Base_ortho,dcosxyz,iaprotoi,ibord,icheck,
     &           iopsymr,isrt,lmaxat,lmaxso,n_atom_proto,natome,
...
!***********************************************************************
      ...
      subroutine irep_comp(Atom_comp0,green,iopsymr,irep_util,
     &                karact,ngrph,nspino,Repres_comp)
...
! Il n'y a pas de couplage up-down dans le cas suivant
          ...
          do is = 1,nopsm
            ...
            if( abs( aimag(karact(is,irep)) ) > eps10 ) 
     &                              Repres_comp(igrph) = .true.
          end do
        end do
      end do
...
! nr: nombre de points qu'on integre
      ...
      real(kind=db):: a, b, c, d, f1, f2, f3, f4, fac1, fac2, R0, r1,
     &          r2, r3, r4, rap14, rap24, rap34, rm, rp, Radius, tiers
...
! Interpolation d'ordre 3
      ...
      real(kind=db):: a, b, c, d, f1, f2, f3, f4, fac1, fac2, r, r1, r2,
     &                r3, r4, rap14, rap24, rap34
...
! Calcul du volume de la maille elementaire
      ...
      Cal_Volume_maille = axyz(1)*axyz(2)*axyz(3)
     &      * sqrt( 1 - cosa**2 - cosb**2 - cosc**2 + 2*cosa*cosb*cosc )
